...
While these experimental design challenges in molecular population genetic studies have existed for at least two decade, high throughput sequencing brings the added technical challenges of highly variable coverage, missing data, and high per-nucleotide error rates.
...
The wrapper scripts are written against a frozen versions of ANGSD and supporting tools for consistency of analysis. 
...
The data are available at \url{https://figshare.com/articles/Example_Data_tar_bz2/2063442} and can be downloaded from within AGNSD-wrapper using teh command 
...
Individuals are shown in the same order as Table \ref{tab:samples}. For results using other values of \texit{K} see Figure \ref{fig:suppadmix}.}
\label{fig:admixture}
\end{figure}

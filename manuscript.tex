...
A Python installation \ph{Citation} is required for some light, dynamic pre-processing of the data.
...
The visualization portion of ANGSD-wrapper also requires installation of the R packages shiny \citep{shiny}, genomeIntervals \citep{genomeIntervals}, ape \citep{APE}, lattice \ph{citation}, Hmisc \ph{citation}, data.table \ph{citation}, DT \ph{citation}, and shinythemes \ph{citation}.
...
ANGSD-wrapper provides a common configuration file, \ph{renamed} \texttt{Common\_Config}, which holds variables that are likely to remain constant across analyses, including identifiers for chromosomal regions and the paths to project directories.

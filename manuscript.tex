...
Analyses supported by ANGSD-wrapper are shown in table \X, and a detailed flowchart of each of these workflows is shown in Figure \ref{fig:supp2}, and additional details, documentation, a tutorial, and a wiki can be found on the GitHub page: \url{https://github.com/mojaveazure/angsd-wrapper/wiki}.\jri{add a simple table of all analyses supported.}
...
The Shiny server automatically parses ANGSD output files and creates the resulting plot(s) (Figure \ref{fig:theta}), which can be saved using the browser's built in image saving capabilities. \jri{is this really chromosome 10 of maize? looks like only first 10Mb. if so, should mention that in figure legend}

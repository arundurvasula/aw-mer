...
\caption{Admixture analysis for {\it Zea mays} spp. {\it mays}, {\it Zea mays} spp. {\it parviglumas}, and {\it Zea mays} spp. {mexicana} (Mex.) with K=4 source populations. }
\label{fig:admixture}
\end{figure}

As a demonstration of analyses in ANGSD-wrapper, we explore patterns of diversity in a single genomic region of a subset of domesticated maize and wild teosinte. We used resequenced samples from the HapMap2 project  and calculated summary statistics using a 10 megabase region on chromosome 10.\jri{cite Chia 2012 instead. include region \& mean depth, supp. table w/ sample names \& depth. include download link.}

We first use \X\ to estimate the site frequency spectrum (SFS) of both maize and its wild progenitor \texit{Zea mays} ssp. \textit{parviglumis}, assuming an inbreeding coefficient of $F=1$ since these are all highly inbred samples. 
...
The mean $F_{ST}$ in this region is \X, close to the genome-wide reported value of \X \citep{hufford2012comparative} .  
The \X windows in the 1\% tail of highest $F_{ST}$ values include genes several genes identified as potential targets of selection during domestication in \citep{hufford2012comparative} 

We next include two samples of the related wild teosinte \textit{Zea mays} ssp. {mexicana} to assess evidence for admixture.  
Figure \ref{fig:admixture}) identifies previously unreported structure within the domesticated maize samples used in \citep{hufford2012comparative}   
